...
// Hash for StorageType (uint64_t or std::array<uint64_t, N>)
...
// Alpha topology: for each alpha key, list of connected alpha keys via single excitation
template<typename StorageType>
using AlphaTopologyT = std::unordered_map<StorageType, std::vector<StorageType>, StorageHashT<StorageType>>;

// =================================================================================
// 1a) Template: build_hamiltonian_sparse_t — Build sparse H from determinants
// =================================================================================
template<typename StorageType>
Eigen::SparseMatrix<double, Eigen::RowMajor> build_hamiltonian_sparse_t(
    ...
    int verbosity = 0,
    int core_start = -1,
    const std::vector<Eigen::Triplet<double>>* precomputed_core_triplets = nullptr,
    const AlphaTopologyT<StorageType>* alpha_topology = nullptr
);

// =================================================================================
// 1b) Template: partition_pool_t
// =================================================================================
template<typename StorageType>
...
diagonalize_subspace_davidson_t(
    ...
    const IntegralSparsityInfo* sparsity = nullptr,
    const std::string& init_strategy = "warm"
);
...
run_trim_t(
    ...
    const std::string& davidson_init = "warm"
);
