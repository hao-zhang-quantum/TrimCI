...
IterativeWorkflowResult<BitType> iterative_workflow_t(
    ...
    double first_cycle_threshold = params.first_cycle_threshold;
    ...
    double noise_decay = params.noise_decay;
    ...
    // Pre-compute integral sparsity info once (inputs are constant)
    auto sparsity_info = build_sparsity_info(n_orb, h1, eri);
    // Always pass sparsity info — ab_exc_table used for αβ early break even in dense systems
    const IntegralSparsityInfo* sparsity_ptr = &sparsity_info;
    if (sparsity_ptr && verbosity >= 1) {
        log_msg(verbosity, 1, "[C++ Workflow] Sparsity detected: h1=" +
                std::to_string(sparsity_info.h1_sparsity) + ", eri=" +
                std::to_string(sparsity_info.eri_sparsity));
    }

    // Pre-compute double excitation table once (sorted by |h_val| desc for early break)
    auto table_start = std::chrono::high_resolution_clock::now();
    auto double_exc_table = precompute_double_exc_table(n_orb, eri, 1e-10, attentive_orbitals);
    auto table_end = std::chrono::high_resolution_clock::now();
    double table_time = std::chrono::duration<double>(table_end - table_start).count();
    log_msg(verbosity, 1, "[C++ Workflow] Double exc table: " +
            std::to_string(double_exc_table.size()) + " entries in " +
            std::to_string(table_time) + "s");
    ...
    for (int iteration = 0; iteration < max_iterations; ++iteration) {
        ...
        // For normalized_uniform, the actual mode is still hb
        if (pool_build_strategy == "normalized_uniform" || pool_build_strategy == "uniform") {
            pb_params.screening_mode = "hb";
        } else {
            pb_params.screening_mode = pool_build_strategy;
        }
        // Use current variational energy for PT2 denominator (e0 - H_jj)
        // Falls back to params.e0 (default 0) for first iteration
        pb_params.e0 = (current_energy != 0.0) ? current_energy : params.e0;
        ...
        if (pool_build_strategy == "mi" && !current_core_set.empty()) {
            size_t N = current_core_set.size();
            std::vector<double> mi_w(n_orb * n_orb, 0.0);
            std::vector<double> n_occ(n_orb, 0.0);
            std::vector<double> nn(n_orb * n_orb, 0.0);
            for (size_t k = 0; k < N; ++k) {
                double c2 = current_core_coeffs[k] * current_core_coeffs[k];
                std::vector<int> occ(n_orb, 0);
                for (int o : current_core_set[k].getOccupiedAlpha()) occ[o]++;
                for (int o : current_core_set[k].getOccupiedBeta()) occ[o]++;
            ...
            pb_params.screening_mode = "mi";
        }
        
        // First-cycle threshold override: use a smaller threshold for iteration 1
        // to avoid slow threshold-decay retries when initial pool is large
        double effective_threshold = threshold;
        if (iteration == 0 && first_cycle_threshold > 0) {
            effective_threshold = first_cycle_threshold;
            log_msg(verbosity, 1, "[C++] Using first_cycle_threshold=" +
                    std::to_string(first_cycle_threshold) + " for iteration 1");
        }
        ...
        auto pool_result = pool_build_t<BitType>(
            ...
            attentive_orbitals, verbosity, pb_params, sparsity_ptr,
            &double_exc_table
        );
        ...
        auto trim_result = run_trim_t<BitType>(
            ...
            verbosity,
            sparsity_ptr,
            current_core_coeffs,  // warm start Davidson
        ...
        if (noise_strength > 0 && noise_decay < 1.0) {
            noise_strength *= noise_decay;
        }
