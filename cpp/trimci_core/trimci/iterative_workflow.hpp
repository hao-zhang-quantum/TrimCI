...
struct IterativeWorkflowParams {
    ...
    std::string pool_build_strategy = "hb";
    ...
    double overshoot_factor = 1.8;  // Pool ceiling = overshoot_factor * target_size
    int stagnation_limit = 0;  // Stop if core set doesn't grow for N consecutive iterations (0=disabled)
    
    // --- DMRG-style noise ---
    double noise_strength = 0.0;
    double noise_decay = 1.0;  // Multiply noise_strength by this each iteration (e.g. 0.999)

    // --- RNG seed for stochastic steps (currently GPU TRIM partition) ---
    // 0 = auto (wall-clock), non-zero = deterministic. Required if you want
    // reproducible runs or meaningful num_runs ensembles.
    ...
    // "warm"             : use previous coefficients as initial guess (default)
    // "lowest_diag"      : start from the det with lowest H_ii
    // "lowest_diag_noise": lowest H_ii + small random noise (robust for mixed spaces)
    // "random"           : random initial guess
    // "multi_random"     : best of multiple random trials
